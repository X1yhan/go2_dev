...
- **直线球侧向拦截抓取已通**(2026-09-25):`chase_grasp.py`——臂收拢折叠(防倾覆)
  → 狗到球线旁 0.30m 平行等待(不挤球)→ 球接近 → base 系球速预判拦截点(侧向,
  臂不外伸)→ 球穿越瞬间 0.4s 快速闭合 → 抬起;球全程直线运动(0.07 m/s,浮空,
  捕获前不停);**翻倒看门狗**:roll/pitch>0.5 或 z<0.20 立即停车退出
  实测第 2 次尝试抓获 z 0.300→0.420;世界仅剩地面+浮空绿球
